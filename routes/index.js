...
router.post("/appleiap/receipt/validation", function(req, res, next) {
  ...
  if (parseRawRecipt["transaction-receipt"] === undefined) {
    res.send({ result: false, error: "receipt invalid transaction receipt" });
  }

  iap.validate(iap.APPLE, parseRawRecipt["transaction-receipt"], function(
    err,
    appleRes
  ) {
    //에러 발생시 에러 전달
...
function ValidationIAB(packageName, productId, token) {
  return new Promise(function(resolve, reject) {
    let getUrl = `https://www.googleapis.com/androidpublisher/v2/applications/${packageName}/purchases/products/${productId}/tokens/${token}?access_token=${
